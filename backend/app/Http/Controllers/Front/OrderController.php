...
class OrderController extends Controller
{
        ...
        try{
            DB::beginTransaction(); 
                if(!empty($request->cart)){
                    ...
                    return response()->json(['status'=>200,'message'=>'Thanks for order!','data'=>$order]);
                }else{
    ...
    public function orderConfirmationDetails($orderId,Request $request){
    ...
    public function getOrderList(Request $request){
        ...
        $order=$order->where(['user_id'=>$request->user()->id,])->paginate($per_page);
    ...
    public function orderDetail($orderId, Request $request){        
        ...
        )->where(['user_id'=>$request->user()->id])->find($orderId);
        if($order){
            return response()->json(['status'=>200,'message'=>'order found','data'=>$order]);
        }else{
